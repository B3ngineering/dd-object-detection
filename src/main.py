...
def file_or_index(value):
    """Custom argparse type: accepts int (webcam) or valid file path (video)."""
    try:
        ivalue = int(value)
        if ivalue < 0:
            raise argparse.ArgumentTypeError('Webcam index must be non-negative')
        return ivalue
    except ValueError:
...
def run_detection(source, multi_model=True):
    ...
    if multi_model:
        detector = MultiModelDetector(
            model_configs=[
                {"path": "arctic_military.pt", "weight": 1, "priority": 2}, # Custom arctic military model
                {"path": "threat_detection.pt", "weight": 1, "priority": 1}, # Additional threat detection model
                {"path": "yolov8s.pt", "weight": 0.5, "priority": 0}, # CoCo model
            ],
            conf_threshold=0.4,
            iou_threshold=0.5
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("source", type=file_or_index, help="Video file path or webcam index (e.g., 0)")
    parser.add_argument("--multi-model", action="store_true", help="Force multi-model detection (default for video files)")
